...
describe('Row-Level Security (RLS) Isolation', () => {
    ...
    it('should restrict data access strictly to the given group in the ALS context', async () => {
        ...
        const userA = await baseClient.user.create({ data: { name: 'A', email: userEmailA, passwordHash: 'hash', gdprConsentAt: new Date() } });
        const userB = await baseClient.user.create({ data: { name: 'B', email: userEmailB, passwordHash: 'hash', gdprConsentAt: new Date() } });

        const groupA = await baseClient.group.create({ data: { name: 'Group A' } });
        const groupB = await baseClient.group.create({ data: { name: 'Group B' } });

        await baseClient.groupMember.create({ data: { groupId: groupA.id, userId: userA.id, role: MemberRole.creator_admin } });
        await baseClient.groupMember.create({ data: { groupId: groupB.id, userId: userB.id, role: MemberRole.creator_admin } });
        ...
        await appContext.run({ groupId: groupA.id }, async () => {
            ...
            const findGroupB = await db.group.findUnique({ where: { id: groupB.id } });
            expect(findGroupB).toBeNull();
        });

        // 3. Cleanup
        await baseClient.groupMember.deleteMany({ where: { groupId: { in: [groupA.id, groupB.id] } } });
        await baseClient.group.deleteMany({ where: { id: { in: [groupA.id, groupB.id] } } });
        await baseClient.user.deleteMany({ where: { id: { in: [userA.id, userB.id] } } });
    });
});
